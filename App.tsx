...
const SetupScreen = ({ onSetup }: { onSetup: (name: string, email: string) => void }) => {
  const [name, setName] = useState('');
  const [email, setEmail] = useState('');

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if(name && email) onSetup(name, email);
  };
  ...
  const SidebarItem = ({ icon: Icon, label, active, onClick }: any) => (
    <button
      ...
      className={`w-full flex items-center space-x-3 px-6 py-4 transition-colors ${
        active ? 'bg-blue-50 text-blue-600 border-r-4 border-blue-600' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700'
      }`}
    >
      <Icon size={20} />
